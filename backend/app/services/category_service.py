...
from ..schemas.сategory import CategoryResponse, CategoryCreate
...
class CategoryService:
    ...
    def get_category_by_id(self,category_id: int) -> CategoryResponse:
        category = self.repository.get_bu_id(category_id)
        if not category:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f'Categorty with id {category_id} not found'
                )
