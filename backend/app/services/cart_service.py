...
from ..schemas.cart import CartResponce,CartItem,CartItemUpdate,CartItemBase,CartItemCreate
...
class CartService:
    ...
    def get_cart_details(self,cart_data: Dict[int,int]) -> CartResponce:
        if not cart_data:
            return CartResponce(item=[],total=0.0,items_count=0)
        
        product_ids = list(cart_data.keys())
        products = self.product_repository.get_multible_by_ids(product_ids)
        ...
        return CartResponce(items=cart_items,total=round(total_price),items_count=total_items)